...
import { Sparkles, Eye, Flame, Wine, Utensils, Bookmark, Heart } from "lucide-react";
...
export const MenuShowcase: React.FC<MenuShowcaseProps> = ({
  ...
  const [selectedCategory, setSelectedCategory] = useState<"tasting" | "beverage" | "favorites">("tasting");
  const { menuItems, favorites, isFavorite, toggleFavorite } = useLocalData();
  ...
  return (
    <section id="menu" className="relative py-28 px-6 sm:px-8 max-w-7xl mx-auto">
      ...
      <div className="flex justify-center mb-12 relative z-10">
        <div className="inline-flex flex-wrap justify-center p-1.5 rounded-full bg-surface-100/90 border border-white/[0.08] backdrop-blur-md gap-1">
          <button
            onClick={() => {
              sound.playClick();
          ...
          >
            ...
            Saved / Favorites {favorites.length > 0 && `(${favorites.length})`}
          </button>
        </div>
      </div>
      ...
      {selectedCategory === "favorites" && filteredItems.length === 0 ? (
        <div className="text-center py-16 px-4 rounded-2xl bg-surface-100/40 border border-white/[0.05] max-w-md mx-auto">
          <Bookmark className="w-8 h-8 text-gold-400/50 mx-auto mb-3" />
          <h3 className="text-lg font-serif text-zinc-200 mb-2">No Saved Dishes Yet</h3>
          <p className="text-xs font-sans text-zinc-400 mb-6">
            Click the bookmark icon on any dish to save it to your personal tasting preferences. Your saved selection will be stored locally.
          </p>
          <button
            onClick={() => setSelectedCategory("tasting")}
            className="px-6 py-2.5 rounded-full bg-gold-500/20 text-gold-200 border border-gold-400/40 text-xs font-sans uppercase tracking-widest"
          >
            ...
            {filteredItems.map((dish) => {
              ...
              return (
                <motion.div
                  ...
                  className="group relative rounded-2xl bg-surface-100/70 border border-white/[0.08] hover:border-gold-400/50 overflow-hidden cursor-pointer backdrop-blur-md transition-all duration-500 hover:shadow-[0_15px_40px_rgba(0,0,0,0.6)] flex flex-col will-change-transform"
                >
                  {/* Dish Image Box */}
                  <div className="relative aspect-[4/3] w-full overflow-hidden">
                    ...
                    <button
                      ...
                      className={`absolute top-3.5 right-3.5 p-2 rounded-full backdrop-blur-md transition-all duration-300 z-20 ${
                        favorited
                          ? "bg-gold-500 text-black shadow-[0_0_12px_rgba(212,175,55,0.6)] scale-105"
                          : "bg-black/50 text-zinc-300 hover:text-gold-300 hover:scale-110"
                      }`}
                    >
                      <Bookmark className={`w-3.5 h-3.5 ${favorited ? "fill-black" : ""}`} />
                    </button>
                  </div>
